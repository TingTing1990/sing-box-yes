...
set_as_entrance() {
    if [[ ! -f "${SCRIPT_FILE_PATH}" ]]; then
        wget --no-check-certificate -O ${SCRIPT_FILE_PATH} https://raw.githubusercontent.com/TingTing1990/sing-box-yes/main/install.sh
...
download_config() {
    ...
    if [[ ! -f "${CONFIG_FILE_PATH}/config.json" ]]; then
        wget --no-check-certificate -O ${CONFIG_FILE_PATH}/config.json https://raw.githubusercontent.com/TingTing1990/sing-box-yes/main/shadowsocks2022/server_config.json
        if [[ $? -ne 0 ]]; then
